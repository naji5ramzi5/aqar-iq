...
function getNextStatus(current: string): 'PENDING' | 'IN_PROGRESS' | 'COMPLETED' {
...
export default function HomePage() {
  ...
  const [newApartmentCount, setNewApartmentCount] = useState('')
  ...
  const handleAddProperty = async () => {
    if (!newPropertyName.trim() || !newPropertyLocation.trim() || !newApartmentCount) return
    setSubmitting(true)
    try {
      const res = await fetch('/api/properties', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          apartmentCount: parseInt(newApartmentCount),
        }),
      })
      if (res.ok) {
        ...
        setNewApartmentCount('')
        await Promise.all([fetchDashboard(), fetchProperties()])
      }
    } catch {
      toast({ title: 'حدث خطأ أثناء إضافة العقار', variant: 'destructive' })
    }
    setSubmitting(false)
  }

  const handleDeleteProperty = async (id: string) => {
    if (!confirm('هل أنت متأكد من حذف هذا العقار؟ سيتم حذف جميع الشقق والمراحل المرتبطة به.')) return
    try {
      const res = await fetch(`/api/properties/${id}`, { method: 'DELETE' })
      if (res.ok) {
        toast({ title: 'تم حذف العقار' })
        if (view === 'property') setView('dashboard')
        await Promise.all([fetchDashboard(), fetchProperties()])
      }
    } catch {
  ...
  const handleToggleStage = async (stageId: string, currentStatus: string) => {
      ...
      if (res.ok) {
        ...
        if (selectedApartment) {
          setSelectedApartment(prev => {
            ...
            return {
              ...prev,
              stages: prev.stages.map(s => s.id === stageId ? { ...s, status: newStatus } : s)
            }
          })
        }
        if (selectedProperty) {
          setSelectedProperty(prev => {
            ...
            return {
              ...
              apartments: prev.apartments.map(a => ({
                ...a,
                stages: a.stages.map(s => s.id === stageId ? { ...s, status: newStatus } : s)
              }))
            }
          })
        }
          ...
          <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">لوحة متابعة عامة للشركة</p>
              </div>
            </div>
                ...
                <div className="space-y-4 mt-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={newApartmentCount}
                      onChange={e => setNewApartmentCount(e.target.value)}
                      min={1}
                      max={500}
                    />
                  </div>
                  <Button
                    ...
                    disabled={!newPropertyName.trim() || !newPropertyLocation.trim() || !newApartmentCount || submitting}
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white"
                  >
                    {submitting ? <Loader2 className="animate-spin" /> : 'إضافة'}
                  </Button>
                </div>
              </DialogContent>
            </Dialog>
          ...
          {properties.length === 0 ? (
            <Card>
              <CardContent className="p-12 text-center">
                ...
                <p className="text-gray-500 mb-2">لا توجد عقارات بعد</p>
                <p className="text-sm text-gray-400">اضغط على &quot;إضافة عقار&quot; لبدء إدارة المشاريع</p>
              </CardContent>
            </Card>
          ) : (
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-3xl mx-auto px-4 py-6">
              ...
              <div className="divide-y divide-gray-100">
                {stages.map(stage => (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-gray-50 transition-colors text-right disabled:opacity-50"
                  >
                    <div className="flex items-center gap-3">
          ...
          <p className="text-center text-xs text-gray-400 mt-6">
            اضغط على أي مرحلة لتغيير حالتها
          </p>
        </main>
      </div>
    )
